...
{
	texture_load_from_file(sdl, sdl->textures[0], "button.png");
	sdl->sprite_clips[0].x = 0;
	sdl->sprite_clips[0].y = 0;
	sdl->sprite_clips[0].w = 300;
	sdl->sprite_clips[0].h = 200;

	sdl->sprite_clips[1].x = 0;
	sdl->sprite_clips[1].y = 200;
	sdl->sprite_clips[1].w = 300;
	sdl->sprite_clips[1].h = 200;

	sdl->sprite_clips[2].x = 0;
	sdl->sprite_clips[2].y = 400;
	sdl->sprite_clips[2].w = 300;
	sdl->sprite_clips[2].h = 200;

	sdl->sprite_clips[3].x = 0;
	sdl->sprite_clips[3].y = 600;
	sdl->sprite_clips[3].w = 300;
	sdl->sprite_clips[3].h = 200;
}
	...
	while (1)
	{
		while (SDL_PollEvent(&sdl->e))
		{
			if (sdl->e.type == SDL_QUIT)
				close_sdl(sdl);
			for (int i = 0; i < TOTAL_BUTTONS; i++)
				handle_event(sdl->buttons[i], &sdl->e);
		}
		SDL_SetRenderDrawColor(sdl->renderer, 0xFF, 0xFF, 0xFF, 0xFF);
		SDL_RenderClear(sdl->renderer);
		for (int i = 0; i < TOTAL_BUTTONS; i++)
			render_button(sdl, sdl->buttons[i]);
